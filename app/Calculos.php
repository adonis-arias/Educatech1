...
class Calculos
{
    public function getNotaSubComponente($idPeriodo,$idNotaStructure,$idStudent)
    {
        $query = Norta_Value::select('Nota')
                ->where('Nota_structures_id',$idNotaStructure)
    ...
    public function getPromedioPeriodoRango($idRango,$idCurso,$idAlumno)
    {
        ...
        if(!is_null($periodo))
        {
            $componentes = Nota::select('id')->where('course_id',$idCurso)->get();
            $acumPromedios = 0;
            $canPromedios = 0;
            foreach ($componentes as $index1 => $componente) {
                $subComponentes = Nota_Structure::select('id')->where('Nota_id',$componente->id)->get();
                $acumNotas = 0;
                $canNotas = 0;
